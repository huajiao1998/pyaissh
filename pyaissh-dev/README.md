...
>
> 曾经的 `build_single.py split`（单文件 → 域文件）已于 **2026-09-21 移除**：它按"每域一个锚正则"切分，而锚是**域内标识**而非域边界（域文件真正的起点是各自首行，单文件里的 `# ===== [域 NN/12] …` 横幅由 join 生成），所以产物与 `domains/` 不一致；旧实现还会**先清空 `domains/` 再切分**，锚一过期就是"域文件被删空"（真实触发过一次）。成品不需要再切回域文件，故连同 MANIFEST 的锚列一并删除。`unit_artifacts` 仍校验 MANIFEST 的 12 域与域文件齐全。
