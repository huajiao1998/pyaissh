...
| **INV-02** | 八个子命令与参数面：`start`(含 `--attach`/`--ttl`/`--cols`)/`run`(`--cmd`/`--cmd-file`/`--wait-rc`/`--no-wait`)/`send`(`--keep-crlf`)/`read`(`--offset`/`--lines`/`--wait-rc`/`--token`/`--keep-ansi`)/`ctrl-c`(`--force`)/`keys`(`--data`/`--raw`/`--cmd-file`)/`list`/`kill`(`--name`/`--all`/`--keep-dir`)。\*`--no-pty` **已删除**（传了被 argparse 拒绝，见 ENG-10/C1） | `pyaissh session --help` + `--suite live_session` | V3 ✅ |
