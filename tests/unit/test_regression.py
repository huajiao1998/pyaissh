...
ROOT = os.path.dirname(os.path.dirname(HERE))  # tests/unit -> 仓库根
_PYAISSH_PY = os.environ.get("PYAISSH_PY")  # 被测目标覆盖：重构期对 built 产物回归
if _PYAISSH_PY:
    import importlib.util
    _spec = importlib.util.spec_from_file_location("pyaissh", _PYAISSH_PY)
    pyaissh = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(pyaissh)
else:
    sys.path.insert(0, ROOT)
    import pyaissh  # noqa: E402
...
def hit(cmd):
    return bool(pyaissh._SENSITIVE_CMD_RE.search(cmd))
...
def linux_branch(streams):
    # 与 pyaissh._setup_console_utf8 Linux 分支同逻辑（os.name != "nt" 时无法真跑，复制验证）
...
print("\n=== 版本 ===")
check("VERSION 为合法 X.Y.Z 且 >= 1.5", re.match(r"^\d+\.\d+\.\d+$", pyaissh.VERSION) is not None
      and pyaissh.VERSION >= "1.5.0", "got %s" % pyaissh.VERSION)
