...
- **用完必须 `kill`**：会话是 `setsid+nohup` 的**远端常驻进程，不会自己退出**（SSH 断开照跑，**本地关机/断网也不影响它和正在跑的命令**——回头 `read --wait-rc` 能续拿 `exit_code` 与输出，cwd/变量都还在）；`kill` 默认连目录一起删（`--keep-dir` 留日志看现场），`--all` 清该主机全部会话；`list` 给 `age_seconds`/`log_bytes`，挂超 24h 会提醒。清理是**进程树闭包 + `verified` 字段**（不会无声误报"清干净"）
