...
- **Windows Git Bash 下注意**：远程路径参数（`--path`/`--remote`）会被 MSYS 自动转成 Windows 路径，务必用 `./pyaissh` 包装（已内置 `MSYS_NO_PATHCONV=1`）或加 `MSYS_NO_PATHCONV=1` 前缀；直接用 `python pyaissh.py` 时远程路径会被改写并打 WARN。经包装器运行时**本地路径**（`--local /tmp/...` 这类 Unix 风格）会自动经 cygpath 转成真实 Windows 路径（Git Bash 的 /tmp ≠ Windows 的 \tmp）
