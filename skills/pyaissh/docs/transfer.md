# pyaissh upload / download — 传输文件（子文档）

> 这是 pyaissh skill 的**子文档**（位于技能目录 `docs/` 子目录下，按需读取、不随 SKILL.md 自动注入）：契约（stdout 单行 JSON / 退出码 / error 类型）以 SKILL.md 为准，本文只讲传输的完整语义。
...
python3 pyaissh.py upload root@1.2.3.4 --local f.txt --remote /tmp/dir    # 远端是目录 = 放入（scp 语义）
...
- **单文件上传自动创建远端父目录（mkdir -p 语义）**：`upload --remote /x/y/z.bin` 时 `/x/y/` 不存在会**自动逐层创建**（省部署前 mkdir）；**新建了目录时 `warnings` 会提示创建了哪些**（如 `已自动创建远端父目录: /nonexistent_dir_xyz/`）——拼写错误（如 `/usr/loca/bin/x`）造出的垃圾目录树因此可见可发现。注意与尾斜杠语义的区别：尾斜杠 = 明确表达"期望目录"（不存在则报错防笔误），单文件路径 = 路径的一部分（自动建父目录是便利）；父目录已存在时不提示。目录上传的"远程目录自动创建"（见下）是文档明示行为，不逐层提示
- **大文件并行分片传输**：下载 ≥8MB 自动 4 连接；`--parallel 1-8` 显式控制（显式给出时 ≥64KB 即并行）；**上传同参数（v1.5.8 起，显式时 ≥64KB 分片，与 `--resume` 互斥）**；**实际档位见结果 `parallel_used` 字段（下载与上传结果都有）**。单条 TCP 流在高丢包/长 RTT 链路吞吐塌陷（实测单流 ~20KB/s，8 连接 ~6 倍提速），下载慢或 `download_timeout` 就加 `--parallel 8`（不行反试 4/2）。经跳板机时共享一条跳板连接开多条隧道（连接数 = 1 跳板 + k 分片）
