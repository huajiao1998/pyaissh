...
- **技能自带回归测试 + 危险区标注（AI 可维护性）**：新增技能目录 `tests/`（`verify_r3.py` 54 例含 L4 凭据正则矩阵、`v3_sig_unit.py` 极早期信号/进程内复用 3 例，均为纯单元、无网络依赖，随技能分发）；`pssh.py` 顶部代码地图新增 **⚠️ 危险区 D1-D4**（极早期信号顺序 / 进程内复用重置 / 凭据正则片段 / SFTP 看门狗与原子写）标注与关联测试；SKILL.md 维护约定补充回归测试命令。
