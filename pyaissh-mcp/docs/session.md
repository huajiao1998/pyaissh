...
- **看门狗临终带走**：有人 `rm -rf` 掉会话目录时，看门狗在下一个 tick 做**最后一次**自证闭包清理
  （starter / `script` / `bash -i` 一起收）然后自退，不留无主孤儿（`kill --all` 的 argv 扫描仍是
  **没有看门狗时**（`--ttl 0`）的兜底）。杀前**必须自证** argv 仍含本会话路径——pid 文件已经没了，
  记住的 pid 可能已被内核复用成无关进程。两种"不碰"的情况：`watch.pid` 属于别的看门狗、
  或 `meta` 变了（同名新会话已接管）⇒ 直接退出，绝不误杀别人的会话。
- **降级会话不会被回收**：若会话缺 `bash.pid`（init payload 没写成功），看门狗就无法判断
  "命令是否在跑"，于是把"pid 未知"一律视为忙 —— 这种会话不会空闲回收（`list` 会给出告警），
  用完请手动 `session kill`。
