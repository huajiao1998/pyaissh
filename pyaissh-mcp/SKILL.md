...
```bash
python3 pyaissh.py session start h --name work --ttl 10m          # 起会话（--ttl 空闲回收，默认 10m；0=不回收）
python3 pyaissh.py session start h --name work --attach          # 还活着就接上（状态保留），没有才新建
python3 pyaissh.py session run   h --name work --cmd 'cd /opt/app && make -j8'  # 跑一条并等结果（一步一次调用）
python3 pyaissh.py session ctrl-c h --name work                  # 中断执行中的命令（会话不死，状态保留；向 pane 注入 Ctrl-C，`--force`=SIGKILL 前台进程组）
...
python3 pyaissh.py session kill  h --name work                   # 收尾（tmux 会话 + 进程树 + 目录，默认一起清）
```
